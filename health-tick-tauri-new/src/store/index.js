...
export const useAppStore = defineStore('app', {
  state: () => ({
    // 配置
    config: loadFromStorage('healthtick_config', {
      workMinutes: 60,
      breakSeconds: 120,
      eyeCareMode: false,
      dailyGoal: 8,
    ...
    remainingSeconds: 0,
    todayDone: loadFromStorage('healthtick_todayDone', 0),
    currentStreak: loadFromStorage('healthtick_currentStreak', 0),
    maxStreak: loadFromStorage('healthtick_maxStreak', 0),
    breakWarning: '',
    breakSkipCount: 0,
    breakSkipNeeded: 3,
    lastSkipClickTime: null,
    weekData: loadFromStorage('healthtick_weekData', []),
    totalCount: loadFromStorage('healthtick_totalCount', 0),
    todayWorkMinutes: loadFromStorage('healthtick_todayWorkMinutes', 0),
    weekWorkData: loadFromStorage('healthtick_weekWorkData', []),
    ...
    celebrateBadge: null,
    todaySkipCount: loadFromStorage('healthtick_todaySkipCount', 0),
    quietRemainingSeconds: 0,
    completedCycles: loadFromStorage('healthtick_completedCycles', 0),
    // 内部状态
    ...
    lastActiveDate: loadFromStorage('healthtick_lastActiveDate', new Date().toISOString().split('T')[0]),
  ...
  watch: {
    config: {
      handler: (newConfig) => {
        saveToStorage('healthtick_config', newConfig)
      },
      deep: true
    },
    todayDone: (newValue) => {
      saveToStorage('healthtick_todayDone', newValue)
    },
    currentStreak: (newValue) => {
      saveToStorage('healthtick_currentStreak', newValue)
    },
    maxStreak: (newValue) => {
      saveToStorage('healthtick_maxStreak', newValue)
    },
    weekData: (newValue) => {
      saveToStorage('healthtick_weekData', newValue)
    },
    totalCount: (newValue) => {
      saveToStorage('healthtick_totalCount', newValue)
    },
    todayWorkMinutes: (newValue) => {
      saveToStorage('healthtick_todayWorkMinutes', newValue)
    },
    weekWorkData: (newValue) => {
      saveToStorage('healthtick_weekWorkData', newValue)
    },
    todaySkipCount: (newValue) => {
      saveToStorage('healthtick_todaySkipCount', newValue)
    },
    completedCycles: (newValue) => {
      saveToStorage('healthtick_completedCycles', newValue)
    },
    lastActiveDate: (newValue) => {
      saveToStorage('healthtick_lastActiveDate', newValue)
    }
  },
  getters: {
    formattedTime: (state) => {
